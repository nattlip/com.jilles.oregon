        ...
        capabilities: {
            ...
            measure_ultraviolet: {
                ...
                get: function (device_data, callback) {
                    ...
                    console.log('capabilitis get measure_ultraviolet entered')

                    // get the bulb with a locally defined function
                    var device = getDeviceById(device_data);
                    console.log('driver 279 capabilitis get device. measure_ultraviolet  ', device.measure_ultraviolet)
                    console.log('driver 201 device_data  ', device_data)
                    if (device instanceof Error) return callback(device);

                    self.realtime(device_data, 'measure_ultraviolet', device.measure_ultraviolet);

                    // send the dim value to Homey
                    if (typeof callback == 'function') {
                        callback(null, device.measure_ultraviolet);
                    }
                }
    ...
    self.updateCapabilitiesHomeyDevice = function (dev) {
        ...
        for (var i in self.homeyDevices) {
            ...
            if (self.homeyDevices[i].data.id == dev.data.id) {
                ...
                self.homeyDevices[i].measure_ultraviolet = dev.measure_ultraviolet;
                self.homeyDevices[i].alarm_battery = dev.alarm_battery;


                self.realtime(self.homeyDevices[i].data, 'measure_ultraviolet', dev.measure_ultraviolet);
